...
def generate_confusion_data(
    ...
    features, targets = next(loader_iter)
    preds = torch.argmax(model(features.cuda()), dim=1)
  
    for i, (features, next_targets) in enumerate(loader_iter):
      next_preds = torch.argmax(model(features.cuda()), dim=1)
      preds = torch.cat((preds, next_preds))
      targets = torch.cat((targets, next_targets))

    class_labels = (dict(sorted(label_to_idx.items(), key=lambda item: item[1])).keys())    
...
def generate_confusion_matrix(
    ...
    if normalize:
        ...
        # Student code begins here
        ##########################################################################
        divisor = np.expand_dims(np.sum(confusion_matrix, axis=1), axis=1)
        confusion_matrix = confusion_matrix / divisor
...
def generate_accuracy_table(
    ...
    compare_output = np.where(targets == preds, 1, 0)
    accuracy_table = np.mean(compare_output, axis=0)
