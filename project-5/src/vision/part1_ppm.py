...
class PPM(nn.Module):
    ...
    def __init__(self, in_dim: int, reduction_dim: int, bins: List[int]) -> None:
        ...
        for bin_size in bins:
            mod = nn.Sequential(
                nn.AdaptiveAvgPool2d(bin_size),     # break incoming feature map into (bin_size x bin_size) grids
                nn.Conv2d(in_dim, reduction_dim, 1, bias=False),                 # compress the grid to in_dim to reduction_dim channels
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, mod in enumerate(self.features):
            out_list.append(F.interpolate(mod(x), (x.shape[2], x.shape[3]), mode='bilinear',align_corners=True))
