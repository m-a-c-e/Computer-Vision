...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
	...
	j = SOBEL_X_KERNEL.shape[1]

	# Adding padding for Ix
	k_pad = (k - 1) // 2		# padding for rows
	j_pad = (j - 1) // 2		# padding for columns
	image_bw = np.pad(image_bw, ((k_pad, k_pad), (j_pad, j_pad)), 'constant', constant_values=((0,0)))

	Ix = np.zeros((m, n)).astype(np.float32)
	Iy = np.zeros((m, n)).astype(np.float32)
	for i in range(0, m):
		for j in range(0, n):
			# slice the image matrix based on m, n, c
			img_slice_x = image_bw[i - k_pad + k_pad : i + k_pad + k_pad + 1, j - j_pad + j_pad : j + j_pad + j_pad + 1]
			img_slice_y = image_bw[i - k_pad + k_pad : i + k_pad + k_pad + 1, j - j_pad + j_pad : j + j_pad + j_pad + 1]

			value_mat_x = SOBEL_X_KERNEL * img_slice_x
			value_mat_y = SOBEL_Y_KERNEL * img_slice_y

			value_x = np.sum(value_mat_x)
			value_y = np.sum(value_mat_y)
...
def compute_harris_response_map(
	...
	S_det   = S_xx * S_yy - np.square(S_xy)
	S_trace = S_xx + S_yy
	R = S_det - alpha * np.square(S_trace)
...
def nms_maxpool_pytorch(
	...
	# find the median of all mxn scores
	median = np.median(R)

	# replace all elements which are less than median to 0
	R = np.where(R < median, 0, R)

	# convert numpy array to tensor
	R_numpy = np.resize(R, (1, R.shape[0], R.shape[1]))		# since maxpool only takes 3-D or 4-D tensors
	R_tensor = torch.from_numpy(R_numpy)							# 3D tensor

	# perform maxpool
	max_pool = nn.MaxPool2d((ksize, ksize), stride=(1, 1), padding=[(ksize - 1) // 2, (ksize - 1) // 2])
	output   = max_pool(R_tensor)

	# convert it to numpy
	output   = output.detach().cpu().numpy()
	output   = np.resize(output, (output.shape[1], output.shape[2]))
	R_numpy = R_tensor.detach().cpu().numpy()
	R_numpy = np.resize(R_numpy, (R_numpy.shape[1], R_numpy.shape[2]))

	img_binary = np.where(output == R_numpy, 1, 0)
	R_img 	   = img_binary * R

	R_idx = np.where(R_img != 0)
	R_x = R_idx[0]
	R_y = R_idx[1]

	k_scores = R_img[R_x, R_y]
	confidences = []
	x = []
	y = []
	for i in range(0, k):
...
def get_harris_interest_points(
	...
	harris_map = compute_harris_response_map(image_bw)
	# normalize the harris response map
	min_val = np.min(harris_map)
	max_val = np.max(harris_map)
	divisor = (max_val - min_val)
	harris_map = (harris_map - min_val) / divisor

	(x_arr, y_arr, conf_arr) = nms_maxpool_pytorch(harris_map, k, ksize=7)
	(x, y, c) = remove_border_vals(image_bw, x_arr, y_arr, conf_arr)
