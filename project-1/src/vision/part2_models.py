...
class HybridImageModel(nn.Module):
	...
	def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
		...
		kernel = np.reshape(kernel, (1, kernel.shape[0], kernel.shape[1], kernel.shape[2]))
		kernel = np.repeat(kernel, self.n_channels, axis=0)
	...
	def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
		...
		"""
		row_pad = (kernel.shape[2] - 1) // 2
		col_pad = (kernel.shape[3] - 1) // 2
		filtered_image = F.conv2d(input=x, weight=kernel, stride=1, padding=[row_pad, col_pad], groups=self.n_channels)	
	...
	def forward(
		...
		cutoff_frequency = torch.Tensor.numpy(cutoff_frequency)
		cutoff_frequency = int(cutoff_frequency[0])
