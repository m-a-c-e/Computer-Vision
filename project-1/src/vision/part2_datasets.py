...
def make_dataset(path: str) -> Tuple[List[str], List[str]]:
    ...
    for file_name in file_list:
        if file_name[2] == '_':
            if file_name[1] == 'a':
                images_a.append(path + '/' + file_name)
            else:
                images_b.append(path + '/' + file_name)
...
def get_cutoff_frequencies(path: str) -> List[int]:
    ...
    cutoff_frequencies = []
    for l in lines:
        cutoff_frequencies.append(int(l))
...
class HybridImageDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        image_a = self.transform.__call__(self.transform, Image.open(self.images_a[idx]))
        image_b = self.transform.__call__(self.transform, Image.open(self.images_b[idx]))
